...
class FlashSR(DDPM):
    def __init__(
        ...
        student_ldm_state_dict = torch.load(student_ldm_ckpt_path, map_location='cpu', weights_only=False)
        self.load_state_dict(student_ldm_state_dict)

        self.vae = VAEWrapper(autoencoder_ckpt_path)

        self.sr_vocoder = SRVocoder()
        sr_vocoder_state_dict = torch.load(sr_vocoder_ckpt_path, map_location='cpu')
