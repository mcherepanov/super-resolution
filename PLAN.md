...
### Удаление из input/
- [x] «Выделить все» / «Удалить выделенное» — `POST /input/delete-selected`
- [x] пропуск файлов в `queued`/`processing`; worker input не трогает

### Экспорт MP3
- [x] битрейт 320 / 256 / 192 / 128 kbps (`mp3_bitrate`, default 320)
- [x] `libmp3lame -b:a` в `ffmpeg_ops.export_audio`
...
## Этап 8 — UX Web UI (простой / эксперт) ✅

### Простой и экспертный режим
- [x] чекбокс **«Эксперт»** в шапке карточки «Обработка» (`index.html`)
- [x] по умолчанию **простой** — видны AI и экспорт; ffmpeg-блоки скрыты (`expert-only` + `hidden`)
- [x] состояние в `localStorage` (`sr_ui_expert_mode`), переживает HTMX swap `#process-panel`
- [x] те же скрытые блоки в CUE-пайплайне (`expert_fieldsets.html`, `compact` для CUE)

### Подсказки для неспециалистов
- [x] кнопка **?** (`opt_tip.html`, CSS `.opt-tip`) у блоков и опций эксперта
- [x] общий partial `expert_fieldsets.html` — основная форма и CUE без дублирования текстов

### Блок AI
- [x] `ai_fieldset.html` — AI + lowpass перед AI
- [x] lowpass **всегда виден**; disabled, пока AI выключен (`initProcessForms` в `app.js`)
- [x] подсказки ? у AI и lowpass

### Input / скачивание (доработки UI)
- [x] batch ZIP: `super-resolution_<UTC>.zip`, iframe `download-frame`
- [x] скрин Web UI в `docs/images/web-ui.png`, README обновлён (пайплайн, скачивание, удаление)
...
- [x] lowpass перед AI в Web UI (`enhance_lowpass`, только при `ENHANCE_AVAILABLE`; чекбокс всегда в блоке AI)
...
### UI и инфраструктура
- [x] простой / экспертный режим UI (см. этап 8)
- [x] подсказки ? у опций ffmpeg в эксперте
