...
| Простой / эксперт | Чекбокс **«Эксперт»** в шапке блока «Обработка»; по умолчанию **простой** — только AI и экспорт |
| Обработка | Чекбоксы файлов; в эксперте — ffmpeg-цепочка, AI, экспорт |
| Подсказки (эксперт) | Кнопка **?** у блоков и опций — когда включать шум, EQ, compand и т.д. |
| Очередь | Таблица jobs, poll 3 с, прогресс-бар при `processing` |
| Прервать | Кооперативная отмена (`cancelled`) |
| AI + lowpass | Чекбокс FlashSR; **lowpass перед AI** всегда виден, активен при включённом AI |
...
### Простой и экспертный режим

Переключатель **«Эксперт»** — в одной строке с заголовком «Обработка» (вне `#process-panel`, не сбрасывается при HTMX-обновлении списка файлов). Выбор сохраняется в `localStorage` (`sr_ui_expert_mode`).

| Режим | Что видно в форме |
|-------|-------------------|
| **Простой** (по умолчанию) | Файлы в `input/`, блок **AI-улучшение**, блок **Экспорт**, кнопка очереди |
| **Эксперт** | То же + шумоподавление, EQ, динамика, нормализация (и в CUE-пайплайне) |

Скрытие — только CSS (`expert-only` + класс `hidden`), поля остаются в форме с дефолтами («Не применять»).

В эксперте у каждой опции — **?** с кратким пояснением: что делает фильтр и нужен ли он для типичного рипа.

### Блок AI

- **С AI-улучшением** — основной режим проекта (FlashSR).
- **Lowpass перед AI** — всегда отображается под чекбоксом AI; неактивен, пока AI выключен. Для узкополосных записей (сильно сжатый MP3, телефония); для обычной музыки обычно не нужен.
...
По умолчанию выключен. Включать для узкополосного входа (телефония, сильно сжатый поток). В UI — чекбокс «Lowpass перед AI» в блоке AI (активен при включённом FlashSR).
